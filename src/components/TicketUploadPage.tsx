...
export default function TicketUploadPage() {
  ...
  const processSingleImage = async (id: string) => {
    ...
    try {
      // 1. Use client-side AI to extract
      const extracted = await extractTicketData(imgData.dataUrl);
      ...
      let rentalId = '';
      if (extracted.plateNumber) {
        const q = query(collection(db, 'rentals'), where('plateNumber', '==', extracted.plateNumber));
      ...
      const ticketPayload = {
        plateNumber: extracted.plateNumber || 'UNKNOWN',
        violationDate: extracted.violationDate ? Timestamp.fromDate(new Date(extracted.violationDate)) : serverTimestamp(),
        amount: Number(extracted.amount) || 0,
        violationType: extracted.violationType || 'Unknown Violation',
                ...
                <div className="flex-1 min-w-0 space-y-3">
                  ...
                  {img.status === 'success' && img.extractedData && (
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div><span className="text-slate-400">Plate:</span> <span className="font-mono font-bold text-slate-700">{img.extractedData.plateNumber}</span></div>
                      <div><span className="text-slate-400">Date:</span> <span className="font-bold text-slate-700">{img.extractedData.violationDate}</span></div>
                      <div><span className="text-slate-400">Amount:</span> <span className="font-bold text-rose-600">${img.extractedData.amount}</span></div>
                      <div><span className="text-slate-400">Type:</span> <span className="font-bold text-slate-700">{img.extractedData.violationType}</span></div>
                    </div>
                  )}
